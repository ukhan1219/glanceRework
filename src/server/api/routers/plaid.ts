...
//     createLinkToken: protectedProcedure.input(z.object({ clientUserId: z.string() })).mutation(async ({ input }) => {
...
//             },
//             //  TODO: in dev/prod, add this as clientuser.name
//             client_name: 'Plaid Test App',
//             products: [Products.Auth, Products.Transactions], // TODO: add more products for statements, investments, etc
//             language: 'en', 
...
//     exchangePublicToken: protectedProcedure.input(z.object({ publicToken:  z.string() })).mutation(async ({ input }) => {
...
//             //  TODO: save accesstoken into db so user does not have to log in each time
...
//     getAccountBalance: protectedProcedure.input(z.object({ accessToken: z.string() })).mutation(async ({ input }) => {
...
//     syncTransactions: protectedProcedure.input(z.object({ accessToken: z.string(), cursor: z.string().optional() })).mutation(async ({ input }) => {
...
//             throw new  Error("Error syncing transactions");
//         }
//     }),

//     getTransactions: protectedProcedure.input(z.object({  accessToken: z.string(), startDate: z.string(), endDate: z.string() })).mutation(async ({ input }) => {
